<template>
  <DefaultLayout>
    <div class="bg-blue-200 min-h-screen flex flex-col large-font">
      <div class="container mx-auto p-4 flex-grow">
        ...
        <div v-if="showEditForm && editingTeam" id="edit-team-form" class="bg-white p-6 rounded-lg shadow-lg mb-6">
          ...
          <form @submit.prevent="handleUpdateTeam" class="space-y-4">
            <div>
              <label for="teamName" class="block text-lg font-large text-gray-700">Team Name</label>
              <input
                type="text"
                id="teamName"
            ...
            <div>
              <div class="flex justify-between items-center mb-2">
                <label class="block text-lg font-large text-gray-700">Add New Players</label>
                <button
                  type="button"
                ...
                <div v-for="(player, index) in editingTeam.newPlayers" :key="index" class="flex items-center space-x-2 p-3 bg-gray-50 rounded-md">
                  <div class="flex-grow">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                      <div>
                        <label :for="`edit-player-name-${index}`" class="block text-lg font-large text-gray-500">Player Name</label>
                        <input
                      ...
                      <div>
                        <label :for="`edit-player-number-${index}`" class="block text-lg font-large text-gray-500">Jersey Number</label>
                        <input
              ...
              <div v-if="showEditExistingPlayers && availablePlayers.length > 0" class="mt-4">
                <div class="flex justify-between items-center mb-2">
                  <label class="block text-lg font-large text-gray-700">Manage Existing Players</label>
                  <button
                    type="button"
                ...
                <div class="max-h-60 overflow-y-auto border border-gray-200 rounded-md p-2">
                  <div v-for="player in availablePlayers" :key="player.id" class="flex items-center py-2 border-b border-gray-100 last:border-0">
                    ...
                    />
                    <label :for="`edit-player-${player.id}`" class="ml-2 block text-lg text-gray-900">
            ...
            <div class="flex space-x-4">
              <button
                type="submit"
                class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-lg font-large rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                :disabled="loading"
              >
              ...
              <button
                type="button"
                @click="cancelEditTeam"
                class="inline-flex justify-center py-2 px-4 border border-gray-300 shadow-sm text-lg font-large rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
              >
                Cancel
              </button>
            </div>
            ...
            <div>
              <div class="flex justify-between items-center mb-2">
                ...
                <button
                  type="button"
                  @click="addNewPlayer"
                  class="inline-flex items-center px-3 py-1 border border-transparent text-lg leading-4 font-large rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                >
                ...
                <div v-for="(player, index) in newTeam.players" :key="index" class="flex items-center space-x-2 p-3 bg-gray-50 rounded-md">
                  <div class="flex-grow">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                      ...
                      <div>
                        <label :for="`player-number-${index}`" class="block text-lg font-large text-gray-500 text-lg">Jersey Number</label>
                        <input
              ...
              <div v-if="showExistingPlayers && availablePlayers.length > 0" class="mt-4">
                <div class="flex justify-between items-center mb-2">
                  <label class="block text-lg font-large text-gray-700">Add Existing Players (Optional)</label>
                  <button
                    type="button"
                ...
                <p class="mt-1 text-lg text-gray-500">{{ newTeam.playerIds.length }} existing players selected</p>
              </div>
        ...
        <!-- Adjust the loading logic to ensure skeletons are hidden when data is available -->
        <!-- Single consistent loading UI -->
        <div v-if="loading && teams.length === 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div v-for="i in 3" :key="`skeleton-${i}`" class="bg-white p-6 rounded-lg shadow-lg animate-pulse">
        ...
        <!-- No teams message -->
        <div v-if="!loading && teams.length === 0" class="bg-white p-6 rounded-lg shadow-lg mb-4">
        ...
        <!-- Teams display -->
        <div v-if="teams.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div
          ...
          >
            <div class="flex justify-between items-start">
              <div class="flex-grow">
                ...
                <div class="mb-4">
                  <p class="text-lg text-gray-600">Manager: {{ team.manager?.displayName || 'No Manager' }}</p>
                </div>
                ...
                <div class="mt-4">
                  <div class="flex justify-between items-center mb-2">
                    ...
                    <button
                      ...
                      class="text-md text-blue-600 hover:text-blue-800"
                    >
                  ...
                  <div class="space-y-2">
                    ...
                    <p v-else class="text-gray-500 text-md">No players in this team</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div v-if="showTeamDetails" class="fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center z-50">
          <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-2xl">
            ...
            <div>
              <p class="text-2xl text-gray-600 mb-4">Manager: {{ selectedTeam.manager?.displayName || 'No Manager' }}</p>
              <div>
                <h3 class="text-xl font-semibold mb-2">Players ({{ selectedTeam.players?.length || 0 }})</h3>
                <div class="space-y-2">
  ...
  <div v-if="showPlayerEditForm" class="fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center z-50">
    <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
      ...
      <form @submit.prevent="savePlayerEdit" class="space-y-4">
        <div>
          <label for="playerName" class="block text-lg font-large text-gray-700">Player Name</label>
          <input
            type="text"
            id="playerName"
        ...
        <div>
          <label for="jerseyNumber" class="block text-lg font-large text-gray-700">Jersey Number</label>
          <input
            type="number"
            id="jerseyNumber"
        ...
        <div>
          <label for="playerTeam" class="block text-lg font-large text-gray-700">Team</label>
          <select
            id="playerTeam"
...
export default {
  ...
  setup() {
    ...
    const { players, getPlayer, updatePlayer, deletePlayerById } = usePlayers()
    ...
    const initialLoadComplete = ref(true) // Remove the need for separate initial load tracking
    const skeletonCount = ref(3) // Keep this for reference but we'll use a simpler approach
    ...
    onMounted(async () => {
      try {
        ...
        // Start fetching teams immediately - single fetch based on role
        const fetchTeamsPromise = user.value?.role === 'Manager'
          ? fetchTeams({ myTeams: true })
          : fetchTeams()

        // Shorter minimum loading time (100ms instead of 200ms)
        ...
        if (user.value?.role === 'Manager') {
          ...
          // Load players more efficiently instead of with a long delay
          nextTick(() => {
